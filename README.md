...
- 每个模型独立计价（输入/输出分别计价，单位：元/百万 token）。
- 每个模型组独立倍率（`rate_milli` 千分比，1000 = 1.0×）；请求时取 Key 所属启用分组中包含该模型的最大倍率，模型原始单价存库不改写，只在预留与结算时折算。
...
- 供应商（渠道）：增删改查、拉取上游 `/v1/models` 预览并勾选导入（幂等，导入为停用未定价）。
- 模型：增删改查、单模型计价、启用/停用。
- 模型组：增删改查、倍率设置、组内模型增删。
